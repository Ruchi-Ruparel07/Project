...
{
	$i = $_POST['d'];
	$qs = "SELECT * from ONLINECANCEL where id = '".$i."'; ";	
	$rst = mysql_query($qs);
	$row = mysql_fetch_row($rst);
	$qd = "DELETE from ONLINECANCEL where id = '".$i."'; ";	
	mysql_query($qd);
}
?>
<h2>User Online Cancellation Table</h2>
...
<?php
$q_sel = "SELECT * FROM ONLINECANCEL order by id";
...
if(isset($_POST['s_edit']))
{
	print "<pre>";
	$i = $_POST['e'];
	$q_sel = "SELECT * from ONLINECANCEL where id = '".$i."' order by id;";
	...
	?>
    <h4>User Online Cancellation Update Form</h4>
